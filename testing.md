...
| **Citation integrity gate** | `pipeline/validate_citations.py` (+ `tests/test_citations.py`, parametrized per race) | The permanent, always-on sibling of the row above — deterministic, zero-network, and never mutates: fails loudly (nonzero exit, one line per violation) on any unsourced/empty/whitespace/non-http(s) source or ungrounded-in-the-candidate's-own-data (fabrication) citation, anywhere across every race's factual bullets, horizons `now`/`long_term` bullets, and candidate finance/key_vote/position records | pre-commit, pre-demo | <1s |
