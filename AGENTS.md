...
- **LLM runtime (optional live path)**: OpenRouter (OpenAI SDK), model `anthropic/Codex-sonnet-4.5`, used only when a race has no precomputed insights.
...
- Commit at every milestone and at every agent completion. **Update the AGENTS.md Status log with EVERY push** — one line: what shipped, where it lives. **Push ONLY to the `data-backend` branch — NEVER push to `main`.** `main` moves via PR merges only. Fetch origin and merge `origin/main` into `data-backend` regularly to stay conflict-free.
- **Commits carry NO AI attribution — no `Co-Authored-By: Codex`, no "Generated with Codex" lines.** Author is the repo's configured git user, plain conventional messages.
