...
- **Precompute-first**: everything precomputable IS precomputed at build time — the gold dataset, Postgres load (or gold-JSON fallback), and per-race insight content (base + 8 voter archetypes) in `data/tx/insights/{race_id}.json`. Runtime = cache reads + Census geocoder (itself cached in `data/geocode_cache.json` or Postgres `geocode_cache` table) + optional live LLM only on cache miss. **The demo works with zero LLM key.**
...
- Commit at every milestone and at every agent completion. **Update the AGENTS.md Status log with EVERY push** — one line: what shipped, where it lives. **Current branch: `postgres-migration`.** Merge to `main` via PR when ready. Fetch origin and merge `origin/main` into working branch regularly to stay conflict-free.
- **Commits carry NO AI attribution** — no `Co-Authored-By:` lines, no "Generated with..." notes. Author is the repo's configured git user, plain conventional messages.
- Never commit `.env` or `data/geocode_cache.json`.
...
- 2026-07-03: Root README rewritten around ground truth + full civic-match surface list; data-backend section untouched. Pushed with AGENTS.md rule compliance.
- 2026-07-04: Full-site audit (profiles, scenarios, graph, stakes, code) → 6 GitHub issues filed and assigned: #6 data gaps (SulaymanB2024), #7 ground-truth violations (lolout1), #8 API hardening (lolout1), #9 UX polish (golkelj), #10 ballot e2e + Railway deploy (AayushBaniya2006), #11 neutrality eval (SulaymanB2024).
- 2026-07-04: Pushed latest data-backend work to main per direct user request: merged remote API/docs/geocode-seed updates, preserved Civitas hero polish, added guided `/ballot` matching flow, and validated Google Civic + hardening tests, frontend lint, and Next build.
- 2026-07-03: Prepared direct main push per user request: preserved frontend Civitas polish, ballot desktop sizing, API fallback/accessibility polish, Google Civic election guardrails, sourced live-insight validation, Postgres insight payload hashing, normalized cached insight caveats, and cleaned generated Playwright output from the worktree.
- 2026-07-04: **Documentation consolidation**: deleted CLAUDE.md (100% duplicate of AGENTS.md), fixed SQLite references (removed deprecated `data/cache.db`/`data/tx/app.db` mentions), corrected model name to `anthropic/claude-sonnet-4.5`, updated git policy to reflect `postgres-migration` branch workflow, verified GitHub issue #7 complete (validate-ground-truth.ts exists and passing), verified issue #21 tooling exists (demo_readiness_gate.py).
- 2026-07-04: **Politicians DB expansion** (postgres-migration): new `data/politicians/` gold set — all 537 Congress members (bioguide/FEC ids, committees, social), 7,453 state legislators for 50 states+DC+PR (Open States bulk CSVs), 26 TX mayors + Houston/SA/Dallas/Austin councils (Wikidata API; Fort Worth council skipped — no API source). Fetchers in `pipeline/fetch_*.py`. Civic-match sparse-data hardening shipped: `lib/coverage.ts` tiers, insufficient-data scoring states, LLM short-circuits on empty evidence, data_quality markers in backfill scripts (build/validate/lint PASS). Still pending: state-executives fetcher run, datastore/API wiring for politicians, geo+horizons insights.
- 2026-07-04: **UI overhaul started** on `ui-overhaul` branch (git worktree): full frontend alignment/polish pass, surfacing missing features, user-focused flow.
- 2026-07-04: **Auto-research feature shipped** (postgres-migration): one-click "Research Now" button appears whenever candidate has insufficient data (<8 stances, missing qualitative). Triggers full agent swarm (profile + 4 issue clusters + qual + promises + finance, all parallel) using government-backed sources (FEC, Congress.gov, House Clerk). Real-time SSE progress updates, auto-reload on completion. New endpoint `/api/check-research/:id`. Docs: `civic-match/docs/AUTO_RESEARCH.md`. Committed `fd9d9e8`.
- 2026-07-03: Synced `data-backend` with `origin/main`, preserved ballot overflow polish + unique JSON temp-file writes, and pushed the branch to `origin/data-backend`.
- 2026-07-03: Pushed latest `data-backend` updates: ballot dashboard sizing, results-card accessibility/link polish, legacy `issue_stances` match/scoring compatibility, architecture presentation source/PDF, and new motivation cache data.
