...
- **Precompute-first**: everything precomputable IS precomputed at build time — the gold dataset, the Postgres load (or gold-JSON fallback), and per-race insight content (base + 8 voter archetypes) in `data/tx/insights/{race_id}.json`. Runtime = cache reads + Census geocoder (itself cached in `data/geocode_cache.json`) + optional live LLM only on cache miss. **The demo works with zero LLM key.**
...
- 2026-07-04: "Fix it all" sweep on postgres-migration: lib/store.ts now falls back to file-DB when DATABASE_URL unset (atomic tmp+rename writes — app works zero-infra locally, Postgres on Railway); wrote missing scripts/import-to-postgres.ts (npm run import:pg); ground-truth violations fixed (4 unsourced scenario facts demoted, 25 structural graph edges marked derived) + scripts/validate-ground-truth.ts (npm run validate) enforcing no-source-no-claim across 96 profiles/4 trees/287 edges — PASSING; issue #6 data gaps closed (nathan-sheets re-researched w/ party, paxton 4th qual dim, goodwin 8 promises, dixon 1 donor — sparse-but-honest); #8 quick fixes: motivate cache keyed on data version, QA question injection-delimited, /api/match prefs sanitized. Build green.
- 2026-07-04: Railway deploy fixed: root package.json/package-lock.json (unused @openrouter/agent, MCP eval leftover) made Nixpacks pick the Node provider — npm ci only, no pip, uvicorn missing, /healthz never came up. Removed both, added nixpacks.toml (providers=["python"]) + .python-version (3.12). Verified /healthz 200 locally (46 races/96 candidates/46 insights).
- 2026-07-04: Prod is LIVE on Railway (project pretty-prosperity): PR #18 merged → web Active at web-production-17c3f.up.railway.app; Railway Postgres provisioned, DATABASE_URL=${{Postgres.DATABASE_URL}} on web, gold data loaded via load_postgres.py (46 races/96 candidates/126 insight blocks). DB connection PROVEN end-to-end: prod ballot call wrote geocode_cache row 2→3. Duplicate empty Postgres-cQhU service deleted via API (orphan volume delete accepted, may lag in UI). schema.md synced to reality (datastore is Postgres-when-set, SQLite gone, deploy section documents the Nixpacks Python pin).
- 2026-07-04: Data-backend docs synced to live prod: railway.md (current-prod-state table, web/Postgres names, Nixpacks Node-provider gotcha), database.md (prod-loaded note, real loader output), README.md (prod URL + Postgres live), DEMO_PLAYBOOK.md engineering beat ("live in production on Railway... served out of PostgreSQL"), abheek.md (prod line). QUALITY.md/data.md clean, no changes needed.
